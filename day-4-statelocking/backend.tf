...
# Run first terraform-7:30am/day-4-resources_for_backend_s3_dynamodb to create reources 
# This backend configuration instructs Terraform to store its state in an S3 bucket.
 terraform {
backend "s3" {
    bucket         = "terraformtfstatesbucket"  # Name of the S3 bucket where the state will be stored.
    region         =  "us-east-1"
